...
The data I will be look into is downloaded and extracted from [Kaggle](https://www.kaggle.com/c/bike-sharing-demand/data). This bike share rental data of Capital Bikeshare only contains entries sampled from Washington D.C. spanning two years dating from January 1st, 2011 to December 19th, 2012. 
...
- "holiday", containing Boolean expressions in $1$s and $0$s representing whether the day of the observation is a holiday or not; 
- "workingday", containing Boolean expressions in $1$s and $0$s representing whether the day of the observation is a working day or not; 
...
# correlation plot
bike.select <- data.frame(bike$day, bike$hour, bike$season, as.integer(bike$isholiday), as.integer(bike$weathertype), bike$temperature, bike$humidity, bike$windspeed, bike$count)
names(bike.select) <- c("day", "hour", "season", "isHoliday", "weathertype", "temperature", "humidity", "windspeed", "count")
...
ggplot(melt(cor(bike.select)), aes(x = Var1, y = Var2, fill = value)) +
  geom_tile() +
...
bike.select2 <- data.frame(seq(1,10886), bike$temperature, bike$humidity, bike$windspeed, bike$count)
names(bike.select2) <- c("hour", "temperature", "humidity", "windspeed", "count")

ggsave("000_correlation_matrix.png", 
       ggplot(melt(cor(bike.select2)), aes(x = Var1, y = Var2, fill = value)) +
         geom_tile() +
         scale_fill_gradient2("", low = WtoGrange(100), mid = RtoWrange(100), high = "gray") +
         geom_text(aes(label = round(value, 2))) +
         coord_flip() + 
